#!/usr/bin/env python
#Last-modified: 04 Sep 2014 11:52:33 AM
...
class FastD(Fasta):
    ...
    def toFastC(self, method='exclusive',sthreshold = 0.05, vthreshold = 0.05):
        ...
        if method in ['f','fisher']:
            Ssum = sum(self.loops)
            Vsum = sum(self.stems)
            S = numpy.array([fisher_exact(x,alternative='greater')[1] for x in zip(zip(self.loops,Ssum-self.loops),zip(self.stems,Vsum-self.stems))])
            V = numpy.array([fisher_exact(x,alternative='less')[1] for x in zip(zip(self.loops,Ssum-self.loops),zip(self.stems,Vsum-self.stems))])
...
class IO(object):
    ...
    def readDataToFastD(fafile,S,V,maxlen=300,ntrim=5,threshold = 0.05, prefix=None):
        ...
        # Read fasta file and generate FastD
        print >> sys.stderr, "Readling fasta file and generating FastD list ..."
        starttime=time.clock()
        FastDlst = []
        FastClst = []
        i0 = 0
        with open(fafile) as fh, open(prefix+".fp", 'w') as fhp, open(prefix+".fc", 'w') as fhc:
            for fa in IO.FastaReader(fafile):
                ...
                if maxlen == 0 or len(fa.seq) <= maxlen:
                    tfp = FastD(fa.name,fa.seq,S1[fa.name],V1[fa.name])
                    # scipy.stats.fisher_exact and FDR
                    fdr_S = Algorithm.fdr_bh([fisher_exact(x,alternative='greater')[1] for x in zip(zip(tfp.loops,Ssum-tfp.loops),zip(tfp.stems,Vsum-tfp.stems))])
                    fdr_V = Algorithm.fdr_bh([fisher_exact(x,alternative='less')[1] for x in zip(zip(tfp.loops,Ssum-tfp.loops),zip(tfp.stems,Vsum-tfp.stems))])
                    # p value and fdr
        ...
        print >> sys.stderr, "{0} sequences passed the maxlen filter.".format(len(FastDlst))
        print >>sys.stderr, "Time used: %-3.2f secs.\n" % (time.clock()-starttime)

        print >> sys.stderr, "Done."
        return (Ssum, Vsum, FastDlst, FastClst)
    readDataToFastD=staticmethod(readDataToFastD)
...
class Predictor(object):
    ...
    def Fold(tfastc, T = 37, threshold = 0,**kwargs):
        ...
        with os.fdopen(fdcn, 'w') as fh:
            fh.write(Utils.FastDToFold(tfastc.constraints))
        os.close(fdct)
        structures = []
        scores = []
        try:
    ...
    def mfold(tfastc, T = 37, threshold = 0,**kwargs):
        ...
        with os.fdopen(fdfa, 'w') as fh: # with statement will close the fh and fd in the end
            fh.write(">{0}\n{1}\n".format(tfastc.name,tfastc.seq))
        with os.fdopen(fdcn, 'w') as fh:
            fh.write(Utils.FastDToMfold(tfastc.constraints))
        structures = []
        scores = []
        try:
            # mfold 
    ...
    def sfoldext(tfastc,T=37,threshold = 0, **kwargs):
       ...
       ''' 
        ...
        try:
            ...
            p = Popen(cmd,stdin = None, stdout = PIPE, stderr = PIPE)
            pstdout, pstderr = p.communicate()
            if pstderr and not pstderr.startswith('Read'):
                raise IOError("ERROR: {0} sampling failed for {1}.".format(os.environ['SCLASSEXE'],tfastc.name))
            if debug: 
                stop2 = time.time()
...
class Algorithm(object):
    ...
    def EM(fd,fs,centroids=None,isoform=None,threshold = 1e-4, maxiter=100):
        ''' 
        ...
        Parameters:
            ...
            isoform: tuple or None
                None means no isoform
                isoforms in format: ((E1,n1),(E2,n2),...,(El,nl)).Ei is expression level and ni is number of structures for the ith isoform.
            threshold: float
        ...
        S = numpy.array(fd.loops)
        V = numpy.array(fd.stems)
    ...
    EM=staticmethod(EM)
    def fitness(fd,fs,threshold=0.05):
        '''
        ...
        Parameters: 
            ...
            treshold: float
                ...
                Set threshold to 0 if all structures are wanted.
        Output:
            fitS: float
        ...
        if threshold > 0:
            idx = idx[numpy.array(scores)>=threshold]
        ...
        for i in idx:
            ...
            IV |= (s !='.') & (s !='-')
        # fitness scores
        loops = numpy.array(fd.loops)
        stems = numpy.array(fd.stems)
        S = sum(loops)
        V = sum(stems)
        fS = sum(loops[IS])
        fV = sum(stems[IV])
    ...
    def MDS(sims, metric = False):
        ...
        Parameters:
            sims: numpy.ndarray
                similarity matrix
            metric: bool
                do metric MDS or non metric MDS
        Returns:
            pos: numpy.ndarray
                an Nx2 matrix, containing coordinates for the data in 2D. 
            y: numpy.array
                second dimension
        '''
        ...
        pos = mds.fit(nsims).embedding_
        return pos
    MDS=staticmethod(MDS)
...
class Utils(object):
    ...
    def FastDToMfold(constraints):
        ''' Convert FastD constraints to mfold constraints. '''
        mconstraints = []
        ...
        return '\n'.join(mconstraints)
    FastDToMfold=staticmethod(FastDToMfold)
    def FastDToFold(constraints):
        '''
        Convert FastD constraints to Fold (RNAStructure package) constraints.
        ...
        fconstraints +="\n-1\nMod:\n-1\nPairs:\n-1 -1\nFMN:\n-1\nForbids:\n-1 -1"
        return fconstraints
    FastDToFold=staticmethod(FastDToFold)
    def ct2dot(ctname):
        ''' 
